...
subtitle: "Slides can be found at https://sianbladon.github.io/coding_best_practice/"
author: "Dr Siân Bladon & Dr Alex Pate"
format: 
  revealjs: 
...
<br> Good coding practice is important because it helps ensure that your code is:
...
<br> Use functions to avoid repeating lines of code <br> General rule of thumb is that if you copy and paste a section of code more than twice then you should make a function
...
<br> You can use the `purrr` package to apply a function to multiple items in a list. <br> E.g. here we are creating a plotting function we want to apply to multiple subsets of the data
...
<br> Then we use the `map` function from `purrr` to iterate through a list of trust names <br> <br>
...
<br> Insert a new section using ctrl + shift + R and navigate using the document outline on the right of the script
...
<br> Working within an R Project is a good way to organise not only your R scripts but keeps all the data and outputs from your work in the same place. <br><br> Avoids the need to use `setwd()` at the start of your scripts, which is not best practice, particularly when collaborating with others.

## Organising Your Work

<br> `setwd()` uses absolute file paths, e.g.
...
<br> Use the source() function to call and run other R scripts within your current script. <br> <br> For example, you may want to automatically run your data cleaning script before your analysis script
...
<br> <br> Or, you may have a separate script with functions you want to use. This helps keep your code clean and easy to follow
...
<br> Objects created while running analyses can be saved using `saveRDS()`.

```{r}
#| echo: TRUE
#| output: FALSE
#| eval: FALSE
saveRDS(my_object, "data/my_object.rds")
```

## Organising Your Work

<br> and read back in using `readRDS()`

```{r}
#| echo: TRUE
#| output: FALSE
#| eval: FALSE
my_object <- readRDS("data/my_object.rds")
```

<br> This allows you to break up analyses into multiple scripts, meaning you don't have to re-run everything when you start a new session.

<br> Useful when working with large datasets or running simulations which take a long time, or running programs non-interactively.

## Organising Your Work

<br> Entire workspace can be saved using `save.image()`

```{r}
#| echo: TRUE
#| output: FALSE
#| eval: FALSE
save.image("data/my_workspace.RData")
```

## Organising Your Work

<br> And read back in using `load()`
...
<br> Advantage: Means you don't have to individually save all research objects that you want to keep.

<br> Disadvantage: Will end up saving stuff you don't need, less space efficient than .rds files, and slower to read in.

## Organising Your Work

<br> Figures can be saved directly into the specified directory:

```{r}
#| echo: TRUE
#| output: FALSE
#| eval: FALSE
png("figures/myplot.png")
myplot
dev.off()
```
...
<br> For ggplots you can use ggsave:

```{r}
#| echo: TRUE
#| output: FALSE
#| eval: FALSE
ggsave(myplot, "figures/myplot.png")
```
...
\- Comment regularly <br>

\- Following naming and formatting conventions

<br> <br> Functions <br>

\- Avoid duplication of code <br>
...
\- Separate folders for code, data, figures <br>

\- Use relative file paths <br>

\- Save research objects to avoid re-running analyses <br>

Reproducibility and transparency <br>

\- Share code on GitHub <br>

\- Write a readme <br>

\- Version control if code is live
